...
penguins <-  penguins
glimpse(penguins)
levels(penguin$island) # it will tell you names for factor variables 
```
...
penguins %>%
  ...
  summarise(mean_bill_length = mean(bill_length_mm, na.rm = T),
            ...
            n=n())
 # filter(!is.na(body_mass_g)), <- pull out all observations that have real number and leave the NA's
```
...
penguins %>% 
  group_by(island) %>% 
  summarize(n=n())    # less fast way to do counts
```
...
```{r}
penguins %>%
  summarize(across(c(species, island, sex), n_distinct)) #variables of interest, then the command 
```

This is very helpful for continuous variables.
```{r}
penguins %>%
  summarize(across(contains("mm"), mean, na.rm=T)) # wowzas thats a good command
```
