...
msleep24 <- msleep %>%
  ...
  arrange(- sleep_total_24) # can also "arrange(desc( xyz ... ))"
msleep24
```
...
```{r}
# dplyr did what I expected, the correct columns were selected and arranged in asccending originally but I added a "-" and now it is correct 
summary(msleep24)
```
...
msleep %>%
  ...
  summarise(mean_sleep_lg=mean(sleep_total),
            ...
            total=n())  #number of observations (rows)
```
...
msleep %>% 
  ...
  summarise(mean_body_wt = mean(bodywt),
...
```{r}
msleep %>%
  summarise(n_count = n_distinct(genus))  #number of different genuses in dataframe 
```
...
```{r}
msleep %>%
  group_by(vore) %>% #we are grouping by feeding ecology, a categorical variable. Groups together CATAGORICAL variables before running data
...
```{r}
msleep %>%
  select(name, order, genus, brainwt)
