...
```{r}
map_shark <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom=7)
...
```{r}
ggmap(map_shark) +
  geom_point(data= sharks, aes(longitude, latitude), size= 0.2, color= injury)  + 
  labs(title = "Locations of Shark Incidents", 
       x= "Longitude", 
       y= "Latitude")
...
lat3 <- c(32.85, 39.58)
long3 <- c(-117.3, -123.8)
bbox3 <- make_bbox(long3, lat3, f=0.04)
```

```{r}
map_shark_fatal <- get_stadiamap(bbox3, maptype = "stamen_terrain", zoom=7)
```

```{r}
ggmap(map_shark_fatal)
```

```{r}
ggmap(map_shark) +
  geom_point(data= sharks_fatal, aes(longitude, latitude), size= 0.6)  + 
  labs(title = "Locations of Shark Fatalities (1950-2022)", 
       x= "Longitude", 
       y= "Latitude")
```
