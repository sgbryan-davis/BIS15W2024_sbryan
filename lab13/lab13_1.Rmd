...
```{r}
#join_type(firstTable, secondTable, by=columnTojoinOn) # this is the syntax
...
inner_exampleDF # inner join mathces observations for which all columns are present. Missing data for 3 and 4
```
...
left_exampleDF #where theres mathces on left
```
...
right_exampleDF #matches on the right
```
...
anti_exampleDF #helps troubleshoot?
```
...
```{r}
spiders_with_locs <- left_join(spiders_all, spiders_locs, by="Accession") #first one is the "left" second one is the "right"
```
...
```{r}
day <- today() #convert dates to factors 
day
```
...
```{r}
datetime <- now()
datetime #record when youre working
```
...
```{r}
dmy(spiders_with_locs$Date) #converts into a format thats easier for R to read, but erases dates not formatted right
```
