...
names(homerange)
```
Using glimpse to view the data types
```{r}
...
- Changing the class of variable
```{r}
...
owls_v2 <- select(owls, "mean_mass_g", "log10_mass", "family", "genus", "species")
min(owls_v2$mean_mass_g)
subset(owls_v2, mean_mass_g == 61.32)
...
max(birds$mean_hra_m2)
subset(birds, mean_hra_m2 == 2.41e+08)

# INFO
# This bird is known as the "Northern crested caracara", and is known as an carnivous ground scavanger. It displays vulture-like feeding habits. It's main habitat comprises of Central and South America, and it is a low conservation concern.
```
