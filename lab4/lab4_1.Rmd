...
#readr is specifying the package it comes from
```
...
# very helpful for managing big data
## `select()`
...
#select(name of data fram, variable(s) of interest)
```
...
```{r}
select_if(fish, ~!is.numeric(.))
# "!" means "not", "(.) means search thru all the data
```
...
anyNA(mammals)
summary(mammals)
# weird how there is -999.00 grams masses
```
...
```{r}
rename("Genus"= "genus",  "AFR"="afr", "max. life"="max_life", "litter size"="litter_size", "litters/year"="litter_per_year")
new_mammals <- rename(mammals, genus="Genus", wean_mass="wean mass", max_life="max. life", litter_size="litter size", litter_per_year="litters/year")
```
...
```{r}
mammals_eff <- select(new_mammals, -"order", -"family")
glimpse(mammals_eff)
```
...
```{r}
select(new_mammals, where(is.character))
select_if(new_mammals, is.character)
# allows to pull variables based on type of data in columns (both work, where() is newer?)
```
...
********
```{r}
# toupper() and tolower() will set column names to upper or lowercase
```
...
names(mammals)
clean_names(mammals)
# makes renaming columns super easy beacuse I will auto replace spaces with underscores
```
