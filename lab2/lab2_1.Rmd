...
y <- 30
y=30 
# ALWAYS USE <- instead of = beacause down the line this will lead to errors
```
 ##
```{r}
...
treatment <- 36
control <- 38
# bad form to use x, y, and z as objects 
```

Here we make a new object `my_experiment` that is the sum of the treatment and control. Notice that I use `_` and not spaces.  
```{r}
my_experiment <- sum(treatment, control) # new object is the sum of treatment and control
my_experiment. # prints out value of sum of objects
```

```{r}

# "TAB COMPLETE" is your friend
# objects are indicated by a purple rectangle

#alternate way of summing
treatment+control
```
# thousands of functions in R, dont give name of object the name of a function

```{r}
mean <- 20. #BAD BAD BAD BAD BAD because it will technically but wont work correctly 
```
...
```{r}
((venom_gt + veyron + chiron)/ 3)

```

3. Use the function `mean()` to calculate the mean top speed for the cars.
```{r}
mean(venom_gt,  veyron,  chiron) #does not work
#use this method instead
carz <- c(venom_gt, chiron, veyron) #c "concatinate. sticks the objects together int a new object
mean(carz)
#ORRRRR you can do
mean(c(venom_gt, chiron, veyron)) #MOST EFFICIENT FOR TYPING
...
```{r}
my_numeric <- 42 #most common type of data, very accurate number
my_integer <- 2L #adding an L automatically denotes an integer, an integers are ONLY whole number
...
```{r}
class(my_numeric)  #some functions work with some classes of data but not others
#"class()" allows to know what each type of data is called
```
...
```{r}
my_integer <- 
  as.integer(my_numeric) #create a new object specified as an integer, create an integer
```
...
#NA's will stop the funtion from working, the output of the function tells me that data is missing
...
```
# very very very helpful piece of code "na.rm=T" remove NA = TRUE,
