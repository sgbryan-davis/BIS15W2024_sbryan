...
```{r}
my_vector_sequence <- c(1:100) #creates integers!!!!
my_vector_sequence
```
...
```{r}
my_vector_sequence[15] #pulls out based on POSITION, does not evaluate for position
my_vector_sequence == 15 #searches for value equal to 15, logical evaluation of the data. Displays T/F
```
...
# vectors then combined vector then format into a matrix then name rows and columns then you can add columns and names to matrix
...
```{r}
harry_potter_matrix <- matrix(box_office, nrow = 8, byrow = T) # data matrix is a bunch of stacked vectors. to access values, you use their coordinates
harry_potter_matrix
```
