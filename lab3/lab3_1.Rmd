...
library("tidyverse")
# data frames = spreadsheets, data frames store multiple types of data
```
...
#data frames give a lot more information in a clearer way
hbirds
```
...
```{r}
names(hbirds)
# can open data frames in workspace
```

What are the dimensions of the `hbirds` data frame? The `dim()` and `str()` commands provide this information.   
```{r}
# dimensions of data frame, columns = variables, observations go in cell
#all variables in lowercase and underscore *NOTHING ELSE*
dim(hbirds)
str(hbirds)
```

```{r}
# rename variables " old_data <- " overwrites old data
hbirds <- data.frame(sex=Sex, lenghth=Length, weight_g=Weight)
```
...
```{r}
hsprings <- data_frame(scientist=name, temp_c=temp, spring)
