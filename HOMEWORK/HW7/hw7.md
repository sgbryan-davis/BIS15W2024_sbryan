...
```r
amniota <- read_csv("amniota.csv") %>% clean_names()
```
...
```r
amphibio <- read_csv("amphibio.csv") %>% clean_names()
```
...
```r
# no NA's in the amniota data, but it would appear they are instead represented by -999, -999.0
miss_var_summary(amphibio)
```
...
```r
# there are about 6600-1000 NA's per variable in this set of data. The high amounts of NA's makes sense for the data because there are several binary variables that were left blank instead of being zeros.
```
...
amniota_fixed <- read_csv("amniota.csv", na=c("-999"))
```
...
```r
vis_dat(amniota_fixed)
```

![](hw7_files/figure-html/unnamed-chunk-7-1.png)<!-- -->
...
```
    The data for mammals makes sense. Mammals do not ley eggs, so they would not have any data for egg mass. Bird eggs would be easier to study based on the size of birds, while reptile eggs would be more difficult due to the small size of many reptiles.
...
## 4 ter        1104     16.3
```
    The NA's in this section of data are meant to represent the zeros of binary variables. The presence of NA's, instead of "0"'s  make it more difficult to do statistical testing on the data.
