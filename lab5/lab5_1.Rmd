...
#Filter by length greater than 350 AND in lake AL
```
...
# gives all fish with Lake ID is AL OR length greater than 350
```
...
```{r}
#lophocebus filtering
lophocebus <- filter(mammals, genus=="Lophocebus" )
lophocebus_2 <- select(lophocebus, "mass", "gestation", "newborn")
```

```{r}
erythrocebus <- filter(mammals, genus=="Erythrocebus" )
erythrocebus_2 <- select(erythrocebus, "mass", "gestation", "newborn")
```

```{r}
macaca <- filter(mammals, genus=="Macaca" )
macaca_2 <- select(macaca, "mass", "gestation", "newborn")
```

### BETTER WAY ???

#make one selected list to filter off of
```{r}
#one way to do this is feed outputs of one commmand into the input  
```
