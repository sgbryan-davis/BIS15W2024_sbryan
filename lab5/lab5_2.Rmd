...
```
# the %>% allows us to nest commands within each other
```{r}
fish %>% # use fish data
  select(lakeid, scalelength) %>% # select the variables wanted
  filter(lakeid == "AL") # filter out based on the selected variables
```
...
fish %>%
  ...
  filter(lakeid == "AL" | lakeid== "AR") %>% #use and/or symbols correctly
