...
1.  The `ui` function controls the user inputs and the way the app will be displayed; i.e. this controls how your app looks.  
  user interface. controlls how apps work and look
...
2.  The `server` function is the part of the app which takes the values of the user inputs, performs calculations and/or makes plots, and prepares the outputs for display; i.e. this controls how your app works.  
    What does the work. controls what app does
3.  The `run` function combines the ui and server function to run the app.  
    Combines ui & server to make it run.
...
```{r, eval=F, echo=T}
ui <- fluidPage( #this is what the user interacts with
...
ui <- fluidPage(
    ...
    plotOutput("plot", width="500px", height="400px") #provides size/aspect of the visual
)
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
      ...
      ggplot(data=penguins, aes_string(x=input$x, input$y, color = "species")) + 
      geom_point() +
...
```{r}
homerange %>%
  ggplot(aes(locomotion, fill=trophic_guild)) +
...
shinyApp(ui, server). #pulls the server and ui data together to actually launch and run the app
```
