...
```{r}
p + theme_linedraw()+ # makes the graph outputs look mor clean, many themes to pick from
...
```{r}
display.brewer.pal(7,"Purples") #sequential palette
```
...
```{r}
q + scale_colour_brewer(palette = "YlOrRd") #must be using "color = "
```
...
```{r}
my_palette <- paletteer_d("dichromat::BluetoOrangeRed_14")
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_wrap(~trophic.guild)+ #splits plots in 2
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_grid(.~trophic.guild)+ #splits plots in 2, but in rows, not columns. the .~ allows choosing between rows and columsn
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_grid(trophic.guild~thermoregulation)+ #splits plots in 2, but in rows, not columns. the .~ allows choosing between rows and columns
  theme(axis.text.x = element_text(angle = 60, hjust=1))
# I can make multple comparitive plots in one go
```
